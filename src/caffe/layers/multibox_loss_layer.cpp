...
namespace caffe {
  ...
  void MultiBoxLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    ...
    if (multibox_loss_param_.only_anchor_match())
    {
      vector<int> overlap_shape(2);
      overlap_shape[0] = 1;
      overlap_shape[1] = num_gt_ * num_priors_;
      pre_gt_overlap_.Reshape(overlap_shape);
    }
  ...
  void MultiBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    ...
    const bool only_anchor_match = multibox_loss_param_.only_anchor_match();
    if (only_anchor_match)
    {
      const Dtype* prior_data_gpu = bottom[2]->gpu_data();
      const Dtype* gt_data_gpu = bottom[3]->gpu_data();
      Dtype* pre_gt_overlap_gpu = pre_gt_overlap_.mutable_gpu_data();
      ComputeOverlappedGPU_1(num_gt_ * num_priors_, prior_data_gpu, 4, 0, \
        gt_data_gpu, 8, 3, num_priors_, pre_gt_overlap_gpu);
      const Dtype* pre_gt_overlap_cpu = pre_gt_overlap_.cpu_data();
      FindMatches(all_loc_preds, all_gt_bboxes, prior_bboxes, prior_variances,
        multibox_loss_param_, &all_match_overlaps, &all_match_indices_, pre_gt_overlap_cpu);
    }
    else
    {
      FindMatches(all_loc_preds, all_gt_bboxes, prior_bboxes, prior_variances,
