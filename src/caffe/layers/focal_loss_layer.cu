...
namespace caffe {
	...
	void FocalLossLayer<Dtype>::compute_intermediate_values_of_gpu() {
		...
		/*--------------------------------------加权重--------------------------------------*/
		//if (has_weights_) {
		//	int count = weights_.count();
		//	caffe_gpu_mul(
		//		count,
		//		weights_.gpu_data(),
		//		log_prob_.gpu_data(),
		//		log_prob_data);  // log(p_t) := w * log(p_t)
		//}
		/*--------------------------------------加权重--------------------------------------*/

		/*****************************************************************************/
		// a.原始focal loss: power_prob_data = (1 - p_t) ^ gamma
		/*caffe_gpu_sub(count, ones_data, prob_data, power_prob_data);
		caffe_gpu_powx(count, power_prob_.gpu_data(), gamma_, power_prob_data);*/

		// b.选择用：power_prob_data = (1 - p_t)
		caffe_gpu_sub(count, ones_data, prob_data, power_prob_data);

		// c.mxnet版本: power_prob_data = alpha_* (1 - p_t) ^ gamma
		/*caffe_gpu_sub(count, ones_data, prob_data, power_prob_data);
		caffe_gpu_powx(count, power_prob_.gpu_data(), gamma_, power_prob_data);
		...
		Dtype gamma)
	{
		CUDA_KERNEL_LOOP(index, nthreads) {
			...
			if (has_ignore_label_ && label_value == ignore_label_) {
				...
				/*****************************************************************************/
				// a.原始focal loss：FL(p_t) = -2*(alpha_t)(1 - p_t) ^ gamma * log(p_t)
				//                   alpha_t = (1 - alpha_) if p_t来自负样本
				//                   alpha_t = alpha_       if p_t来自负样本
				/*if (label_value == 0)
				{
				...
				// b.自己版本：FL(p_t) =(1 - p_t) ^ gamma * log(p_t) if p_t来自负样本
				//           FL(p_t) =(1 - p_t) ^ 1.0 * log(p_t) if p_t来自正样本         
				if (label_value == 0)
				{
				...
				// c.mxnet版本：FL(p_t) = alpha_* (1 - p_t) ^ gamma * log(p_t)
		...
		const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
	{
		...
		/*Dtype normaler;
		if (has_weights_) {
			caffe_gpu_asum(bottom[2]->count(), bottom[2]->gpu_data(), &normaler);
			normaler = normaler / bottom[2]->count();
			top[0]->mutable_cpu_data()[0] = loss / get_normalizer(normalization_,
				valid_count) / normaler;
		}
		else
		{
			top[0]->mutable_cpu_data()[0] = loss / get_normalizer(normalization_,
		...
		CUDA_KERNEL_LOOP(index, nthreads) {
			...
			if (has_ignore_label_ && label_value == ignore_label_) {
				...
				* log_prob_data[ind_i] * prob_data[ind_i]
				...
				// b.选择用：FL(p_t) =(1 - p_t) ^ gamma * log(p_t) if p_t来自负样本
				//           FL(p_t) =(1 - p_t) ^ 1.0 * log(p_t)   if p_t来自正样本         
				...
				// c.mxnet版本：FL(p_t) = alpha_* (1 - p_t) ^ gamma * log(p_t)
				/*Dtype grad = - gamma * (power_prob_data[ind_i] / max(1 - prob_data[ind_i], eps))
				* log_prob_data[ind_i] * prob_data[ind_i]
				+ power_prob_data[ind_i];*/
				/*****************************************************************************/
