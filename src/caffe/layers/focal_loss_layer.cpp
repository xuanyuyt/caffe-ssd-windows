...
namespace caffe {
		...
		const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
	{
		...
		/*if (has_weights_) {
		  ...
		  CHECK_EQ(prob_.count(), weights_.count());
			weights_.set_cpu_data(bottom[2]->mutable_cpu_data());
		}*/
		/*--------------------------------------加权重--------------------------------------*/
	}
	...
	void FocalLossLayer<Dtype>::compute_intermediate_values_of_cpu() {
		...
		//		weights_.cpu_data(),
		//		log_prob_.cpu_data(),
		//		log_prob_.mutable_cpu_data());  // log(p_t) := w * log(p_t)
		//}
		/*--------------------------------------加权重--------------------------------------*/
			...
			for (int i = 0; i < outer_num_; ++i) {
				for (int j = 0; j < inner_num_; ++j) {
					...
					* log_prob_data[ind_i] * prob_data[ind_i]
					...
					// c.mxnet版本：FL(p_t) = alpha_* (1 - p_t) ^ gamma * log(p_t)
					/*Dtype grad = - gamma_ * (power_prob_data[ind_i] / std::max(1 - prob_data[ind_i], eps))
					* log_prob_data[ind_i] * prob_data[ind_i]
					+ power_prob_data[ind_i];*/
					/*****************************************************************************/
